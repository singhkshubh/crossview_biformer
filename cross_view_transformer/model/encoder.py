...
class CrossAttention(nn.Module):
    def __init__(self, dim, heads, dim_head, qkv_bias, norm=nn.LayerNorm):
        ...
        self.routing_act = nn.Softmax(dim=-1) #NEW
    ...
    def forward(self, q, k, v, skip=None):
        ...
        v = rearrange(v, 'b n d h w -> b n (h w) d') # yahan bhi change kiya hai pahle (nhw)
    ...
    #    -------------------------------- CHANGES ------------------------------------
        ...
        # zero=torch.zeros(b,n,sq,l_q,d_q).cuda()
        # one=torch.ones(b,n,sq,l_q,d_q).cuda()#all q set to -inf
        # i=topk_qr.view(b, n, topk_q, 1,1).expand(-1, -1, -1,l_q, d_q)
        # zero.scatter_add_(-3,i,one) #masked querry with just topk_qr elem
        # mask=zero
        ...
        b,n,sq,l,lk=dot.shape
        zero=torch.zeros(b,n,sq,l,lk).cuda()
        one=torch.ones(b,n,sq,l,lk).cuda()
        i=topk_qr.view(b, n, topk_q, 1,1).expand(-1, -1, -1,l, lk)
        zero.scatter_add_(-3,i,one)
        mask=zero
        dot = dot*mask
